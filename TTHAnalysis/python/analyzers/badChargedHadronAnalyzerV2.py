...
class badChargedHadronAnalyzerV2( Analyzer ):

    def __init__(self, cfg_ana, cfg_comp, looperName ):
        super(badChargedHadronAnalyzerV2,self).__init__(cfg_ana,cfg_comp,looperName)

    def declareHandles(self):
        super(badChargedHadronAnalyzerV2, self).declareHandles()
    ...
    def beginLoop(self, setup):
        super(badChargedHadronAnalyzerV2,self).beginLoop( setup )
                ...
                for c in self.handles['packedCandidates'].product():
                    if abs(c.pdgId()) == 211:
                        ...
                        if deltaR( it.eta(), it.phi(), c.eta(), c.phi() ) < maxDR and abs(dPtRel) < minPtDiffRel:
                            ...
                            break
                if flagged: break
        event.badChargedHadronV2=(not flagged) 
        return True

setattr(badChargedHadronAnalyzerV2,"defaultConfig", cfg.Analyzer(
        class_object = badChargedHadronAnalyzerV2,
        muons='slimmedMuons',
