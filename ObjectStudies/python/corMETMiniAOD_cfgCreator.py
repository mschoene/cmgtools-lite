...
useEEModifiedMET=False #create an additionnal EEModifiedMET slimmed MET collection if the option is set to false
...
#     runMetCorAndUncFromMiniAOD(process,
#                                isData=options.isData,
#                                pfCandColl=cms.InputTag("noHFCandst"),
...
if not useEEModifiedMET:        
    print "running modified MET" 
    process.load('Configuration.StandardSequences.FrontierConditions_GlobalTag_cff')
    from Configuration.AlCa.GlobalTag import GlobalTag
    process.GlobalTag = GlobalTag( process.GlobalTag, "94X_dataRun2_ReReco_EOY17_v6")

    runMetCorAndUncFromMiniAOD(process,
                               isData=options.isData, # false for MC
                               fixEE2017 = True,
                               fixEE2017Params = {'userawPt': True, 'PtThreshold':50.0, 'MinEtaThreshold':2.65, 'MaxEtaThreshold': 3.139} , #comment for v1
                               postfix = "ModifiedMET",
                               )

    
    process.EEModifiedMET = cms.Path( process.fullPatMetSequenceModifiedMET )
...
if options.removeResiduals:
    ...
    process.patPFMetT1T2CorrModifiedMET.jetCorrLabelRes = cms.InputTag("L3Absolute")
    process.patPFMetT1T2SmearCorrModifiedMET.jetCorrLabelRes = cms.InputTag("L3Absolute")
    process.patPFMetT2CorrModifiedMET.jetCorrLabelRes = cms.InputTag("L3Absolute")
    process.patPFMetT2SmearCorrModifiedMET.jetCorrLabelRes = cms.InputTag("L3Absolute")
    process.shiftedPatJetEnDownModifiedMET.jetCorrLabelUpToL3Res = cms.InputTag("ak4PFCHSL1FastL2L3Corrector")
    process.shiftedPatJetEnUpModifiedMET.jetCorrLabelUpToL3Res = cms.InputTag("ak4PFCHSL1FastL2L3Corrector")
### ------------------------------------------------------------------
...
process.MINIAODSIMoutput = cms.OutputModule("PoolOutputModule",
                                            ...
                                            outputCommands = cms.untracked.vstring( "keep *_slimmedMETs*_*_*",
                                                                                    ...
                                                                                    "keep *_patPFMetT1TxyModifiedMET_*_*",
